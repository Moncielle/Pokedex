...
//button para cambiar a la siguiente generacion
let arrowRight = document.getElementById('arrow-right').addEventListener('click', e=>{
    if (generationshow < 4){
       ...
       divGeneration.innerHTML = 'Generación ' + generationshow
       drawPokemon()
   }
})

//buton para regresar a la generación
...
function  createPokemon(pokemon, modal){
	...
	if (modal !==true){
        const pokeInnerHTML = `
            ...
            <div class="info">
                <span class="number">No. ${pokemon.id.toString().padStart(3, '0')}</span>
